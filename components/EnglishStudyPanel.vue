<template>
  <div class="action-group english-study-panel">
    <h3 class="action-group-title">英文学习打卡</h3>
    <div class="english-study-body">
        ...
        <div class="english-study-main" :aria-busy="pending">
          <div class="english-study-stats-grid">
            <div class="english-study-stat-card">
              <p class="english-study-stat-label">当前连续</p>
              <p class="english-study-stat-value">
                <span class="english-study-streak-icon">🔥</span
                >{{ currentStreak }} 天
              </p>
            </div>
            <div class="english-study-stat-card">
              <p class="english-study-stat-label">历史最长</p>
              <p class="english-study-stat-value">{{ bestStreak }} 天</p>
            </div>
          </div>

          <div class="english-study-week">
            <p class="english-study-week-title">本周</p>
      ...
      <p
        v-if="saveMessage"
        class="english-study-message"
        :class="{ 'english-study-message--error': saveError }"
      >
        {{ saveMessage }}
      </p>
    </div>
...
const { data, pending, error, refresh } = await useAsyncData(
  "home-english-study",
  () => $fetch<EnglishStudyResponse>("/api/english-study"),
  {
...
const saveButtonLabel = computed(() => {
  ...
  if (saving.value) return "打卡中...";
  if (checkedInToday.value) return "今日已打卡";
  return "今日打卡";
});
...
const todayLabel = computed(() => {
  ...
  if (!year || !month || !day) return today.value;

  const weekdayLabels = ["日", "一", "二", "三", "四", "五", "六"];
  const weekday =
    weekdayLabels[new Date(Date.UTC(year, month - 1, day)).getUTCDay()];
  return `${year}年${month}月${day}日 周${weekday}`;
});

async function saveCheckin() {
  if (checkedInToday.value || pending.value) return;
